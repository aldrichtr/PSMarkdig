  ...
  try {
    ...
    try {
      ...
      try {
        ...
        Write-Verbose "File '$Path' sample is not valid UTF-8"
      }
      ...
      # NOTE: Modified the fallback to $null
      # return New-Object System.Text.UTF8Encoding($false)
      return $null
    } finally {
      $stream.Close()
    ...
    Write-Verbose "Error detecting encoding for '$Path': $($_.Exception.Message)"
    # NOTE: Modified the fallback to $null
    # return New-Object System.Text.UTF8Encoding($false)
    return $null
  }
