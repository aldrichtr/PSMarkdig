...
try {
  # SECTION Enable debug if set
  if ($null -ne $env:__PSMODULE_DEBUG) {
    if (($moduleName -like $env:__PSMODULE_DEBUG) -or
      ($env:__PSMODULE_DEBUG -eq $true) -or
      ($env:__PSMODULE_DEBUG -like 'all')) {
  ...
  [ArrayList]$private:sourceFiles = $private:sourceDirectories |
    ForEach-Object {
      Write-Debug "Looking for source files in $_"
  ...
  try {
    :file foreach ($private:file in $remainingFiles) {
      ...
      if ($currentFile -match '\.ps1$') {
        ...
        # NOTE: If you want to add the namespace, name the file `NameSpace.ClassName`
        if ($null -eq $typesToExport) { $typesToExport = [ArrayList]::new() }
        Write-Debug "Using the module $currentFile"
        $typeName = $currentFile | Split-Path -LeafBase
        Write-Debug "Sourcing file $file"
        $sb = [scriptblock]::Create((Get-Content $currentFile -Raw))
        . $sb
        $null = $typesToExport.Add($typeName)
  ...
  if ($typesToExport.Count -gt 0) {
    ...
    foreach ($typeToExport in  $typesToExport) {
      ...
      if (-not $type) {
        $Message = @(
          "Unable to register type accelerator '$typeToExport'"
        ...
        if ($fullTypeToExport -in $existingTypeAccelerators.Keys) {
          $Message = @(
            "Overriding type accelerator '$($typeToExport)' with '$($type.FullName)'"
            'Accelerator already exists.'
          ) -join ' - '

          Write-Warning -Message $Message
        } else {
          Write-Verbose -Message "Added type accelerator '$($typeToExport)' for '$($Type.FullName)'."
        }
  ...
  if ($typesToExport.Count -gt 0) {
    $MyInvocation.MyCommand.ScriptBlock.Module.OnRemove = {
      foreach ($private:typeName in $typesToExport) {
        $null = $TypeAcceleratorsClass::Remove($typeToExport)
      }
    }.GetNewClosure()
  }
